...
public class AES {
    ...
    private static final int[][] inv_a_x = {
            {0x0e, 0x0b, 0x0d, 0x09},
            {0x09, 0x0e, 0x0b, 0x0d},
            {0x0d, 0x09, 0x0e, 0x0b},
            {0x0b, 0x0d, 0x09, 0x0e}
    ...
    private void keySchedule(int[][] curKey) {

        for (int i = 0; i < 4; i++) {
            System.arraycopy(curKey[i], 0, this.roundKeys[i], 0, 4);
        }
    ...
    protected void addRoundKey(int[][] currentKey){
        for (int i = 0; i < 4; ++i){
            for (int j = 0; j < 4; ++j){
                this.state[i][j] = this.state[i][j] ^ currentKey[i][j];
            }
        }
    }
    ...
    protected void subBytes(){
        for (int i = 0; i < 4; ++i){
            for (int j = 0; j < 4; ++j){
                this.state[i][j] = AES.sbox[this.state[i][j] / 16][this.state[i][j] % 16];
            }
        }
    }

    protected void invSubBytes(){
        for (int i = 0; i < 4; ++i){
            for (int j = 0; j < 4; ++j){
                this.state[i][j] = AES.invsbox[this.state[i][j] / 16][this.state[i][j] % 16];
            }
        }
    }
    ...
    public int[][] encrypt(int[][] plainText){
        ...
        addRoundKey(this.key);
        for (int i = 1; i < 10; ++i){
            subBytes();
            shiftRows();
            mixColumns();
            addRoundKey(subKey(i));
        }
        subBytes();
        shiftRows();
        addRoundKey(subKey(10));

        return this.state;
    }

    public int[][] decrypt(int[][] cryptoText){
        keySchedule(this.key);
        addRoundKey(subKey(10));
        for (int i = 9; i > 0; --i){
            invShiftRows();
